...
# 1️⃣ Premier bouton
if "roles_page_loaded" not in st.session_state:
    st.session_state["roles_page_loaded"] = False

def load_roles_page():
    st.session_state["roles_page_loaded"] = True

st.button("🔄 Charger la répartition", on_click=load_roles_page)

if not st.session_state["roles_page_loaded"]:
    st.info("Cliquez sur 🔄 Charger la répartition pour afficher le formulaire.")
    st.stop()
...
with st.form("repartition_form"):
    ...
    if st.form_submit_button("✅ Valider la répartition"):
...
with st.spinner("Chargement des répartitions…"):
